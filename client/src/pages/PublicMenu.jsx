    ...
    <Box sx={{ bgcolor: T.bg, minHeight: '100vh', fontFamily: 'Inter, sans-serif', color: T.text }}>
      ...
      >
        <Box sx={{ maxWidth: 1280, mx: 'auto', px: 3, height: 80, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <Box sx={{ width: 40, height: 40, bgcolor: '#ea580c', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
            ...
            <Box sx={{ display: 'flex', overflowX: 'auto', gap: 1.5, px: 3, '&::-webkit-scrollbar': { display: 'none' } }}>
              {categories.map(cat => (
                <Box component="button" key={cat} onClick={() => setActiveCategory(cat)} sx={{
                  ...
                  transition: 'all 0.3s',
                  ...(activeCategory === cat
                    ? { background: 'linear-gradient(to bottom right, #f97316, #ea580c)', color: '#fff', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }
          ...
          <Box component="section" sx={{ px: 3, mt: 2, display: 'flex', flexDirection: 'column', gap: 4 }}>
            {displayedCats.map((category) => (
              ...
              <M key={category._id} variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }}>
                ...
                <Box sx={{ height: 4, width: 48, bgcolor: '#f97316', borderRadius: '9999px', mb: 3 }} />
                    ...
                    <M key={item._id} variants={scaleUp} custom={itemIdx} sx={{
                      ...
                      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', pt: 1 }}>
                          ...
                          {cart[item._id] ? (
                            <Box sx={{ display: 'flex', alignItems: 'center', bgcolor: 'rgba(249,115,22,0.1)', borderRadius: '9999px', px: 0.5 }}>
                              <Box component="button" onClick={() => removeFromCart(item._id)} sx={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#f97316', background: 'none', border: 'none', cursor: 'pointer' }}>
                                <span className="material-symbols-outlined" style={{ fontSize: 14 }}>remove</span>
                              </Box>
                              <Typography sx={{ px: 1, fontWeight: 700, fontSize: '0.875rem', color: '#f97316' }}>{cart[item._id]}</Typography>
                              <Box component="button" onClick={() => addToCart(item._id)} sx={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#f97316', background: 'none', border: 'none', cursor: 'pointer' }}>
                                <span className="material-symbols-outlined" style={{ fontSize: 14 }}>add</span>
                              </Box>
                            </Box>
                          ) : (
                            <Box component="button" onClick={() => addToCart(item._id)} sx={{
                              width: 40, height: 40, borderRadius: '50%', bgcolor: '#ea580c', color: '#fff7ed',
      ...
      <AnimatePresence>
        {cartCount > 0 && (
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'linear-gradient(to bottom right, #f97316, #ea580c)', color: '#fff', borderRadius: '9999px', p: 2, boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
              ...
              <Box component="button" onClick={goToCheckout} sx={{
                display: 'flex', alignItems: 'center', gap: 1, bgcolor: '#fff', color: '#f97316',
